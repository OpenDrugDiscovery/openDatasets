...
class ReplicaExchange:
    def __init__(self,
        ...
        self.reporter_filepath = os.path.join(self.out_dir, "reporter.nc")
        self.ckp_filepath = os.path.join(self.out_dir, "checkpoint.nc")
        self.energy_data_filepath = os.path.join(self.out_dir, "energy_data.dat")
        self.energy_plot_filepath = os.path.join(self.out_dir, "energy_plot.pdf")
        self.energy_hist_filepath = os.path.join(self.out_dir, "energy_hist.pdf")
        self.states_plot_filepath = os.path.join(self.out_dir, "states.pdf")
        self.states_prob_filepath = os.path.join(self.out_dir, "state_prob_matrix.pdf")
        self.replica_template_fname = os.path.join(self.out_dir, "replica_{}.dcd")
        self.reporter = MultiStateReporter(self.reporter_filepath, checkpoint_interval=1)

    def reporter_to_dict(self):
        reporter = MultiStateReporter(self.reporter_filepath, open_mode="r")
    ...
    def post_processing(self, series_per_page=4, write_data_file=True, equil_nskip=1):
        """
        ...
        if write_data_file == True:
            f = open(self.energy_data_filepath, "w")
            for step in range(total_steps):
                f.write(f"{step:10d}")
        ...
        self.plot_energies(
            ...
            file_name=self.energy_plot_filepath,
        )
        self.plot_energy_histograms(
            state_energies,
            temperatures,
            file_name=self.energy_hist_filepath,
        )
        self.plot_summary(
            ...
            series_per_page,
            time_interval=time_interval,
            file_name=self.states_plot_filepath,
        )
        self.plot_matrix(
            replica_state_indices,
            file_name=self.states_prob_filepath,
        )
    ...
    def save_replica_files(self, timestep=5*unit.femtosecond, time_interval=200,
        frame_begin=0, frame_stride=1, center=False):
        """
        Save dcd files from replica exchange simulation trajectory data.
        ...
        reporter = MultiStateReporter(self.reporter_filepath, open_mode='r', checkpoint_storage=self.ckp_filepath)
        states = reporter.read_thermodynamic_states()[0]
        ...
        for replica_index in range(n_replicas):
            replica_positions = extract_trajectory(self.topology, reporter, replica_index=replica_index,
            ...
            traj_times = np.linspace(0, (n_frames_tot-1)*self.step_time, num=n_frames_tot)
            ...
            Trajectory.save_dcd(replica_traj, self.replica_template_fname.format(replica_index+1))
    ...
    def get_minimum_energy_ensemble(self):
        ...
        # energies = np.zeros()
        print(r_energies.shape)
        print(r_positions.shape)
        print(self.n_replica, self.n_steps)

        nts = r_energies.shape[0]
        d = r_energies[np.arange(nts), np.arange(nts)]
        print(d.shape)
        exit()
...
if __name__ == "__main__":
    ...
    from opendata.conformers.simple_md import get_topology_system
    import openmm 
    ...
    re = ReplicaExchange(topology=topology, 
                         system=system, 
                         positions=positions, 
                         out_dir=opath, 
                         total_time=0.01*unit.nanosecond,
                         step_time=2*unit.femtosecond,
                         overwrite=True)
    re.run()
    re.get_minimum_energy_ensemble()
    # re.post_processing()
